...
6. 在有原生字幕的 YouTube 视频中打开“完整字幕”，确认状态显示段数、字幕覆盖起止时间和语言；“合并条数”、5、10、20 三选一与紧凑的“翻译 / 重试”按钮全部保持在同一行；每组时间范围位于正文上方，默认每 5 条合并，点击时间范围可跳转到该组开头。
...
- Manifest V3 版本为 1.0.19，默认语言为 `zh_CN`。
