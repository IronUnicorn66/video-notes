...
插件支持 YouTube 和哔哩哔哩原生字幕，也可以读取沉浸式翻译已经显示在播放器上的双语字幕。对于 YouTube 已暴露原生字幕轨道的视频，侧栏还会在浏览器本机尝试读取完整字幕，显示字幕覆盖的起止时间，并提供 5、10 或 20 条合并阅读、独立字号调节和时间跳转；若 YouTube 限制返回字幕响应，扩展可能短暂切换并恢复播放器字幕，以捕获播放器发起的原生字幕响应，仍不可用时才提示失败。完整字幕只使用 Edge 内置 Translator API 在侧栏本机翻译。扩展自动识别字幕源语言，首版可选择简体中文、英语、日语、韩语或西班牙语作为目标语言，支持当前语言对提前下载、进度显示和下载后离线使用；下载前不显示大小，完成后显示预估占用。插件不使用第三方字幕翻译服务或远程脚本。
...
6. 在有原生字幕的 YouTube 视频中打开“完整字幕”，确认状态显示段数、字幕覆盖起止时间和语言；“合并条数”、5、10、20 三选一、紧凑的“翻译 / 重试”以及字号 +/− 按钮全部保持在同一行。点击 +/− 只缩放字幕原文和译文，刷新侧栏后保留字号；每组时间范围位于正文上方，默认每 5 条合并，点击时间范围可跳转到该组开头。
7. 在“全文翻译”中确认字幕源语言由完整字幕自动识别且不可编辑；“翻译为”只提供简体中文、英语、日语、韩语和西班牙语五项。选择未下载的当前语言对后不显示大小；点击“下载语言包”可查看进度，完成后显示约 200 MiB 的预估占用。点击完整字幕中的“翻译”，确认显示原文与所选目标语言译文对照；断网后再次翻译仍可运行。
...
- Manifest V3 版本为 1.0.21，默认语言为 `zh_CN`。
...
- 全量自动化测试、构建和 ZIP 完整性校验均通过；此前已在 Edge 150 完成加载与字幕烟测，本批次在 Edge 152 确认本地翻译语言包下载进度，其余人工路径见 `docs/ACCEPTANCE.md`。
