...
插件支持 YouTube 和哔哩哔哩原生字幕，也可以读取沉浸式翻译已经显示在播放器上的双语字幕。对于 YouTube 已暴露原生字幕轨道的视频，侧栏还会在浏览器本机尝试读取完整字幕，显示字幕覆盖的起止时间，并提供 5、10、20 或 30 条合并阅读和时间跳转；若 YouTube 限制返回字幕响应，扩展可能短暂切换并恢复播放器字幕，以捕获播放器发起的原生字幕响应，仍不可用时才提示失败。完整字幕默认使用 Edge 内置 Translator API 在侧栏本机翻译为简体中文，首次语言包下载完成后可离线使用。用户也可主动选择自己的 HTTPS OpenAI 兼容 API 作为备用；扩展不会自动把本地翻译失败回退到云端。插件不使用第三方字幕服务或远程脚本。
...
Edge 的可见页面截图接口要求 `<all_urls>` 或一次临时 `activeTab` 授权。插件只在用户点击“启用播放器截图”并接受 Edge 权限提示后申请；实际截图仍只发生在受支持的 YouTube 和哔哩哔哩视频页。浏览器本地全文翻译不需要额外主机权限；只有用户选择云端备用方式并保存 API 配置或点击翻译时，才为其填写的单一 HTTPS API 主机请求对应子权限。用户可以在 Edge 扩展设置中撤销该权限；侧栏会显示当前授权状态。
...
- 用户输入的笔记正文与编辑历史。
- 当前页面已经渲染的字幕片段。
- 用户使用本地全文翻译时的字幕语言、目标语言代码与 Edge 语言包状态；主动选择云端备用时的当前 YouTube 完整字幕，以及用户填写的 API Base URL、API Key 和模型名。
- 用户主动启用后截取的可见播放器画面。
...
- 不出售、出租或共享用户数据。
- 外部网络请求包括用户主动选择的静态 Whisper 模型权重下载、Edge 首次使用语言对时管理的 Translator 语言包下载，以及用户主动选择云端备用并点击翻译后直接发给其所填 API 的完整字幕。本地 Translator 不发送字幕内容；模型下载服务会收到普通 HTTPS 请求的连接元数据，云端翻译 API 的数据处理由用户选择的服务决定。
...
7. “翻译方式”默认使用浏览器本地翻译。点击“翻译”，确认首次使用显示语言包下载进度，随后显示原文与简体中文译文对照；断网后再次翻译仍可运行。审核人员也可主动选择 OpenAI 兼容 API 备用方式并填写自己的 HTTPS Base URL、API Key 和模型名；未配置时点击翻译应聚焦设置且不发送请求。
...
- Manifest V3 版本为 1.0.17，默认语言为 `zh_CN`。
