...
在浏览器本机保存快捷键、前置字幕设置、完整字幕合并条数和字号、时间线排序、Whisper 模型状态和一次性联动提示。笔记及二进制资产使用扩展自己的 IndexedDB 与 Cache Storage。
...
- 模型选择、下载状态、快捷键、侧栏整体缩放比例、完整字幕合并条数和字号以及时间线排序设置。
...
7. 在“全文翻译”中确认字幕源语言由完整字幕自动识别且不可编辑；“翻译为”只提供简体中文、英语、日语、韩语和西班牙语五项。选择未下载的当前语言对后不显示大小；点击“下载语言包”可查看进度，完成后显示约 200 MiB 的预估占用。点击完整字幕中的“翻译”，确认浏览器按断句后的完整段落翻译、进度按段落计算且翻译期间合并档位不可切换；断网后再次翻译仍可运行。
...
- Manifest V3 版本为 1.0.22，默认语言为 `zh_CN`。
...
- 全量自动化测试、构建和 ZIP 完整性校验均通过；此前已在 Edge 150 完成加载与字幕烟测、在 Edge 152 确认本地翻译语言包下载进度，本批次新增断句与整段翻译验证，其余人工路径见 `docs/ACCEPTANCE.md`。
