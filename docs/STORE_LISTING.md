...
Edge 的可见页面截图接口要求 `<all_urls>` 或一次临时 `activeTab` 授权。插件只在用户点击“启用播放器截图”并接受 Edge 权限提示后申请；实际截图仍只发生在受支持的 YouTube 和哔哩哔哩视频页。用户可以随时在侧栏或 Edge 扩展设置中撤销。
...
- 不出售、出租或共享用户数据。
- 唯一的外部下载是用户主动选择的静态 Whisper 模型权重。

如果表单询问扩展是否访问或处理网站内容、浏览活动或音频，应选择与上述实际行为一致的“是”，并说明处理只发生在本机且服务于公开的单一用途。如果表单询问是否向开发者或第三方收集、传输或共享个人内容，应选择“否”。
...
- Manifest V3 版本为 0.3.0，默认语言为 `zh_CN`。
- ZIP 包含 `_locales/zh_CN/messages.json`。
- ZIP 不包含测试、开发文档、商店素材、Git 元数据或模型权重。
- 包内没有远程 JavaScript 或动态代码加载器。
- 自动化测试、构建、ZIP 完整性和真实 Edge 加载均通过。
- 主页、隐私政策和支持页面无需登录并返回 HTTP 200。
