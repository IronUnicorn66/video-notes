...
插件支持 YouTube 和哔哩哔哩原生字幕，也可以读取沉浸式翻译已经显示在播放器上的双语字幕。对于 YouTube 已暴露原生字幕轨道的视频，侧栏还会在浏览器本机尝试读取完整字幕，显示字幕覆盖的起止时间，并提供每 5 条合并阅读、搜索和时间跳转；若 YouTube 限制返回字幕响应，功能会提示不可用。插件不使用第三方字幕服务或远程脚本。
...
6. 在有原生字幕的 YouTube 视频中打开“完整字幕”，确认状态显示段数、字幕覆盖起止时间和语言；每组时间范围位于正文上方，默认每 5 条合并，搜索时仅显示逐条匹配结果，点击时间范围可跳转到该组开头。
7. 点击麦克风“授权”，在独立授权页再次点击允许并接受浏览器权限提示。返回课程页面后，按住侧栏“按住说话”按钮约两秒，松开后确认生成带原始录音的笔记。
8. 本地 Whisper 为可选功能。选择 Base 并点击“下载并使用”，同意模型主机权限；等待固定权重下载和摘要校验完成，再录制一条语音，确认本地转写进入笔记。
9. 点击“导出 ZIP”，确认下载包包含 Markdown；启用过截图或录音时，包内同时包含对应 `images/` 或 `audio/` 文件。
...
- Manifest V3 版本为 1.0.10，默认语言为 `zh_CN`。
