...
插件支持 YouTube 和哔哩哔哩原生字幕，也可以读取沉浸式翻译已经显示在播放器上的双语字幕。对于 YouTube 已暴露原生字幕轨道的视频，侧栏还会在浏览器本机尝试读取完整字幕，显示字幕覆盖的起止时间，并提供 5、10、20 或 30 条合并阅读和时间跳转；若 YouTube 限制返回字幕响应，扩展可能短暂切换并恢复播放器字幕，以捕获播放器发起的原生字幕响应，仍不可用时才提示失败。用户还可选填自己的 HTTPS OpenAI 兼容 API，在主动点击翻译后将完整字幕直接发送给该服务并显示简体中文译文。插件不使用第三方字幕服务或远程脚本。
...
Edge 的可见页面截图接口要求 `<all_urls>` 或一次临时 `activeTab` 授权。插件只在用户点击“启用播放器截图”并接受 Edge 权限提示后申请；实际截图仍只发生在受支持的 YouTube 和哔哩哔哩视频页。全文翻译则在用户保存 API 配置或点击翻译时，只为其填写的单一 HTTPS API 主机请求对应的子权限。用户可以在 Edge 扩展设置中撤销该权限；侧栏会显示当前授权状态。
...
- 用户输入的笔记正文与编辑历史。
- 当前页面已经渲染的字幕片段。
- 用户点击全文翻译时的当前 YouTube 完整字幕，以及用户填写的 API Base URL、API Key 和模型名。
- 用户主动启用后截取的可见播放器画面。
...
- 不出售、出租或共享用户数据。
- 外部网络请求包括用户主动选择的静态 Whisper 模型权重下载，以及用户主动点击全文翻译后直接发给其所填 API 的完整字幕。模型下载服务会收到普通 HTTPS 请求的 IP、User-Agent、请求时间和模型文件地址等连接元数据；翻译 API 的数据处理由用户选择的服务决定。
...
6. 在有原生字幕的 YouTube 视频中打开“完整字幕”，确认状态显示段数、字幕覆盖起止时间和语言；“合并条数”、5、10、20、30 四选一与紧凑的“翻译 / 重试”按钮全部保持在同一行；每组时间范围位于正文上方，默认每 5 条合并，点击时间范围可跳转到该组开头。
7. 全文翻译需要审核人员自备 OpenAI 兼容 API。可选填 HTTPS Base URL、API Key 和模型名后保存并接受对应主机权限，点击“翻译”确认显示进度、原文与简体中文译文对照；未配置时点击翻译应跳转到设置且不发送请求。
...
- Manifest V3 版本为 1.0.16，默认语言为 `zh_CN`。
...
- 全量自动化测试、构建和 ZIP 完整性校验均通过；用户已在 Edge 150 完成加载、YouTube 原生字幕与沉浸式翻译双语字幕烟测，其余人工路径见 `docs/ACCEPTANCE.md`。
