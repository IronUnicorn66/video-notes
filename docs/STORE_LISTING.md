...
- Manifest V3 版本为 1.0.25，默认语言为 `zh_CN`。
...
- 全量自动化测试、构建和 ZIP 完整性校验均通过；此前已在 Edge 150 完成加载与字幕烟测、在 Edge 152 确认本地翻译语言包下载进度与播放器当前进度定位，本批次新增笔记优先复用当前会话完整字幕及匹配译文验证，其余人工路径见 `docs/ACCEPTANCE.md`。
