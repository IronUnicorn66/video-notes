...
5. 首次需要转写时，在“本地 Whisper”选择模型并点击“下载并使用”，同意一次模型下载权限。下载中的录音或转写任务完成前不能切换模型。
6. 每次转写结果会按模型追加到候选和历史；再次转写不会覆盖已经编辑的个人正文。
7. 时间线中的截图可点击放大并关闭；语音笔记还可播放保存的原始录音。
8. 看完后点击“导出 ZIP”。导出不会删除浏览器中的会话或已下载模型。
