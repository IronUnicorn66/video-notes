...
2. 点击输入框。插件记录当前时点、字幕与截图并暂停视频；输入内容后移开光标即可保存和续播。
3. 页面焦点位于视频时，按住右 Option/Alt 说话，松开后视频立即恢复。未启用 Whisper 时仍会保留原始录音。
4. 首次需要转写时，在“本地语音与快捷键”中点击“启用”，同意一次模型下载权限。
5. 看完后点击“导出 ZIP”。导出不会删除浏览器中的会话。
